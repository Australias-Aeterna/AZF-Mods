...
selectedfactions = [];
if (aaf_Blufor_Stripper) then {selectedfactions pushback WEST;};
if (aaf_indie_Stripper) then {selectedfactions pushback INDEPENDENT;};
if (aaf_Opfor_Stripper) then {selectedfactions pushback EAST;};
if (aaf_Civvy_Stripper) then {selectedfactions pushback CIVILIAN;};
{
if (side _x in selectedfactions) then {
	if !(isPlayer _x) then 
	{
		if (_x isKindOf "CAManBase") then 
		{
			if !(_x getVariable ["stripperExcluder", false]) then 
			{
				_x addEventHandler [
