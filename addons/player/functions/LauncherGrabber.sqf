...
{
	_boxContents = weaponCargo _x;
	_weapon = _boxContents select 0;if (_weapon isKindOf ["Launcher", configFile >> "CfgWeapons"]) exitWith {
		player action ["TakeWeapon", _x, _weapon];
	}
} forEach _holderList;
...
/*
	...
	if (_weapon isKindOf ["Launcher", configFile >> "CfgWeapons"]) then {
		player action ["TakeWeapon", _box, _weapon];
	}
	_holderlist = nearestObjects [player, ["WeaponHolderSimulated"], 5];
	{
		_boxContents = weaponCargo _x;_weapon = _boxContents select 0;if (_weapon isKindOf ["Launcher", configFile >> "CfgWeapons"]) exitWith {
			player action ["TakeWeapon", _x, _weapon];
		}
	} forEach _holderList;
	_holderList = player nearObjects 5; ["WeaponHolderSimulated"];
	{
		_boxContents = weaponCargo _x;_weapon = _boxContents select 0;if (_weapon isKindOf ["Launcher", configFile >> "CfgWeapons"]) then {
			nearbyboxes pushBackUnique _x;
		}
	} forEach _holderList;
