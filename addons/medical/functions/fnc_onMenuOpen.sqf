...
ACEGVAR(medical_gui,menuPFH) = [ACELINKFUNC(medical_gui,menuPFH), 0.1, []] call CBA_fnc_addPerFrameHandler;
...
if (ACEGVAR(medical_gui,target) == ACE_player) then {
    ...
    _ctrl ctrlSetTooltip ACELSTRING(medical_gui,ToggleToOther);
} else {
    _ctrl ctrlSetText QPATHTOF(data\categories\toggle_to_self.paa);
    _ctrl ctrlSetTooltip ACELSTRING(medical_gui,ToggleToSelf);
};
